...
create table modelos_pedido(no_pedido int, id_modelo int, cantidad int not null,detalles varchar(300), subtotal float, primary key(no_pedido, id_modelo),foreign key(no_pedido) references pedidos(no_pedido), foreign key(id_modelo) references modelos(id_modelo));
